...
/*-------------------------------------------------------------------*/
    protected void setBotMovement(Vector3f moveDir, Vector3f aimDir, float vel, int postureState) {
        ...
        velocity.setForward(Math.round(aimDir2f.x * moveDir2f.x + aimDir2f.y * moveDir2f.y));
        velocity.setRight(Math.round(perp.x * moveDir2f.x + perp.y * moveDir2f.y));
...
/*-------------------------------------------------------------------*/
    protected boolean isNearestOpponentVisible(boolean withinFOV) {
        ...
        if (nearestOpponent != null) {
            ...
            if (traceFromView) pos.add(proxy.getWorld().getPlayer().getPlayerView().getViewOffset());
            return (withinFOV ? Utils.calcAngles(dir)[0] <= proxy.getWorld().getPlayer().getPlayerView().getFOV() / 2.0 && bsp.isVisible(pos, oppPos) :
                    bsp.isVisible(pos, oppPos));
        } else
            return false;
    }
