...
/*-------------------------------------------------------------------*/
    private void processSpecialEntity() {
        if (entityType == TemporaryEntity.TE_SPLASH || entityType == TemporaryEntity.TE_LASER_SPARKS || entityType == TemporaryEntity.TE_WELDING_SPARKS
                || entityType == TemporaryEntity.TE_TUNNEL_SPARKS) {
            tempEnt.setCount(data[offset++]);
            tempEnt.setOrigin(readPosition());
            tempEnt.setMoveDir(readDirection());
            tempEnt.setStyle(data[offset++]);
        } else if (entityType == TemporaryEntity.TE_PARASITE_ATTACK || entityType == TemporaryEntity.TE_MEDIC_CABLE_ATTACK
                || entityType == TemporaryEntity.TE_HEATBEAM || entityType == TemporaryEntity.TE_MONSTER_HEATBEAM) {
