...
//		players[mergeState] = players[currentState];
    ...
    private void checkDrownStatus(Player player) {
        ...
        player.drownTTL = (drownTimer == Long.MIN_VALUE ? drownTimer : Math.max(
                drownTimer + 12000 - curTime + (envSuitTimer != Long.MIN_VALUE && curTime - envSuitTimer <= 28000 ? envSuitTimer + 28000 - curTime : 0), 0));
    }
...
/*-------------------------------------------------------------------*/
    public synchronized void setEntity(Entity entity, boolean baseline) {
        if (baseline)
            ...
            deactivatedEntities[entity.getNumber()] = (entities[mergeState][entity.getNumber()] != null && entities[mergeState][entity.getNumber()].getActive()
                    && !entity.getActive());
        ...
        if (sound.getEntityNumber() == playerEntityNum + 1 && pkup > 0 && config.getConfigString(sound.getConfigIndex()).indexOf("pkup") >= 0) {
            if (trackInventory && isPlayerActive()) {
                ...
                if (pkup >= 8 && pkup <= 17) {
                    ...
                    inventory.setCount(PlayerGun.getAmmoInventoryIndexByGun(pkup), Math.min(
                            PlayerGun.getMaxAmmoByGun(pkup), inventory.getCount(PlayerGun.getAmmoInventoryIndexByGun(pkup)) + PlayerGun.getAmmoPerPickupByGun(
                                    pkup)));
                } else if (pkup >= 18 && pkup <= 22)
                    inventory.setCount(pkup, Math.min(PlayerGun.getMaxAmmo(pkup), inventory.getCount(pkup) + PlayerGun.getAmmoPerPickup(pkup)));
                else
                    inventory.setCount(pkup, inventory.getCount(pkup) + 1);
            ...
            if (deactEnts != null) {
                ...
                for (int i = 0; i < deactEnts.length; i++) {
                    ...
                    if (deactEnts[i].getNumber() != playerEntityNum + 1 && deactEnts[i].getNumber() > 0
                            && (curDist = playerOrigin.distance(entOrigin)) < minDist && deactEnts[i].getInventoryIndex() == pkup) {
                        minDist = curDist;
