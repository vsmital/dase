...
public class Entity {
    ...
    public static final String TYPE_INVULNERABILITY = Config.items[Inventory.INVULNERABILITY], TYPE_AMMOPACK = Config.items[Inventory.AMMO_PACK],
            TYPE_QUAD = Config.items[Inventory.QUAD_DAMAGE], TYPE_SILENCER = Config.items[Inventory.SILENCER], TYPE_ADRENALINE
            = Config.items[Inventory.ADRENALINE],
            TYPE_BANDOLIER = Config.items[Inventory.BANDOLIER], TYPE_REBREATHER = Config.items[Inventory.REBREATHER], TYPE_ENVIRONMENTSUIT
            = Config.items[Inventory.ENVIRONMENT_SUIT],
            SUBTYPE_COMBATARMOR = Config.items[Inventory.COMBAT_ARMOR], SUBTYPE_ARMORSHARD = Config.items[Inventory.ARMOR_SHARD], SUBTYPE_BODYARMOR
            = Config.items[Inventory.BODY_ARMOR],
            SUBTYPE_JACKETARMOR = Config.items[Inventory.JACKET_ARMOR];

    public static final String TYPE_ANCIENTHEAD = Config.items[Inventory.ANCIENT_HEAD], SUBTYPE_DATACD = Config.items[Inventory.DATA_CD], SUBTYPE_BLUEKEY
            = Config.items[Inventory.BLUE_KEY],
            SUBTYPE_REDKEY = Config.items[Inventory.RED_KEY], SUBTYPE_POWERCUBE = Config.items[Inventory.POWER_CUBE], SUBTYPE_PYRAMIDKEY
            = Config.items[Inventory.PYRAMID_KEY],
            SUBTYPE_DATASPINNER = Config.items[Inventory.DATA_SPINNER], SUBTYPE_SECURITYPASS = Config.items[Inventory.SECURITY_PASS], SUBTYPE_COMMANDERSHEAD
            = Config.items[Inventory.COMMANDERS_HEAD],
            SUBTYPE_SHELLS = Config.items[Inventory.SHELLS], SUBTYPE_BULLETS = Config.items[Inventory.BULLETS], SUBTYPE_CELLS = Config.items[Inventory.CELLS],
            SUBTYPE_ROCKETS = Config.items[Inventory.ROCKETS],
            SUBTYPE_SLUGS = Config.items[Inventory.SLUGS], SUBTYPE_POWERSCREEN = Config.items[Inventory.POWER_SCREEN], SUBTYPE_POWERSHIELD
            = Config.items[Inventory.POWER_SHIELD];
...
/*-------------------------------------------------------------------*/
    ...
    private static String[] enumerateItemStrings() {
        ...
        itemStrings[0] = CAT_ITEMS;
        itemStrings[1] = CAT_WEAPONS;
        itemStrings[2] = CAT_PLAYERS;
        itemStrings[3] = CAT_OBJECTS;
        itemStrings[4] = TYPE_KEYS;
        itemStrings[5] = TYPE_ARMOR;
        itemStrings[6] = TYPE_HEALTH;
        itemStrings[7] = TYPE_AMMO;
        itemStrings[8] = TYPE_MEGAHEALTH;
        itemStrings[9] = SUBTYPE_MEDIUM;
        itemStrings[10] = SUBTYPE_LARGE;
        itemStrings[11] = SUBTYPE_STIMPACK;
...
/*-------------------------------------------------------------------*/
    public void merge(Entity entity) {
        if (model == null || entity == null) model = (entity == null ? (model == null ? new Model() : model) : entity.model);
        else model.merge(entity.model);
        if (effects == null || entity == null) effects = (entity == null ? (effects == null ? new Effects() : effects) : entity.effects);
        else effects.merge(entity.effects);
        if (origin == null || entity == null) origin = (entity == null ? (origin == null ? new Origin() : origin) : entity.origin);
        else origin.merge(entity.origin);
        if (angles == null || entity == null) angles = (entity == null ? (angles == null ? new Angles() : angles) : entity.angles);
        else angles.merge(entity.angles);
        if (oldOrigin == null || entity == null) oldOrigin = (entity == null ? (oldOrigin == null ? new Origin() : oldOrigin) : entity.oldOrigin);
        else oldOrigin.merge(entity.oldOrigin);
        if (sound == null || entity == null) sound = (entity == null ? (sound == null ? new Sound() : sound) : entity.sound);
        else sound.merge(entity.sound);
        if (events == null || entity == null) events = (entity == null ? (events == null ? new Events() : events) : entity.events);
        else events.merge(entity.events);
        if (solid == null || entity == null) solid = (entity == null ? (solid == null ? new Solid() : solid) : entity.solid);
        else solid.merge(entity.solid);
